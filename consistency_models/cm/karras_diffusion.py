...
class KarrasDenoiser:
    ...
    def denoise(self, model, x_t, sigmas, **model_kwargs):
        ...
        if self.timestep_type == "cm":
            rescaled_t = 1000 * 0.25 * th.log(sigmas + 1e-44) # Origin CM c_noise
        elif self.timestep_type == "edm":
            rescaled_t = sigmas.log() / 4 # EDM c_noise
        else:
            raise NotImplementedError(f"timestep type {self.timestep_type} is not supported.")
